...
import { getSubLocations , getSubSubLocations , getProjectBySubSubLocations} from '../../../action/lookup';
import { getFormDetail, getForms } from '../../../action/company';
import { headerHelper } from '../../../util/ui-helper';

class SubSubLocations extends Component {
    ...
    componentWillMount() {
        const { params } = this.props.navigation.state;
        this.props.getSubSubLocations(this.props.token, this.props.team.id, params.parentId).then(data => {
            this.setState({
                sublocations: data
            });
        });
    ...
    projectClicked(project) {
        this.props.getProjectBySubSubLocations(this.props.token, this.props.team.id, project.id).then(data => {
            if(data && data.length > 0) {
                navigate('ProjectBySubSubLocation', { parentId: project.id, title: project.name, form:this.props.navigation.state.params['form'], path: this.props.navigation.state.params['path']});
            } else {
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        getProjectBySubSubLocations: (token, teamId, parentId) => { return getProjectBySubSubLocations(token, teamId, parentId)}
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(SubSubLocations);