...
import { getSubRegisters } from '../../../action/lookup';
import { getFormDetail, getForms } from '../../../action/company';
import { headerHelper } from '../../../util/ui-helper';

class SubRegisters extends Component {

    navigate = undefined;

    renderHeader() {
        const { params } = this.props.navigation.state;
        params.title = 'SubRegisters';
    ...
    componentWillMount() {
        const { params } = this.props.navigation.state;
        this.props.getSubRegisters(this.props.token, this.props.team.id, params.parentId).then(data => {
            this.setState({
                subregisters: data
            });
        });
    ...
    projectClicked(project) {
        this.props.getSubRegisters(this.props.token, this.props.team.id, project.id).then(data => {
            // console.warn(data)
            // if(data && data.length > 0) {
            //     navigate('SubRegisterForProject', { parentId: project.id, title: project.name,form:this.props.navigation.state.params['form'], path: this.props.navigation.state.params['path']});
            // } else {
            //     //alert('hello')
            //console.log("Project id:", project.id);
                navigate('ProjectItems', {title: project.name, parentId: project.id ,projectId:this.props.navigation.state.params['projectId'],form:this.props.navigation.state.params['form'], path: this.props.navigation.state.params['path']});
            //}
        });
    }
        ...
        return (
            <ScrollView style={styles.container}>
                ...
                <View style={styles.containerInner}>
                    <FlatList data={this.state.subregisters} style={styles.cardList} keyExtractor={(item) => item.key}
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        getSubRegisters: (token, teamId, parentId) => { return getSubRegisters(token, teamId, parentId)}
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(SubRegisters);